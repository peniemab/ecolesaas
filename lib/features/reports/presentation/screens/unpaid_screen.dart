...
class _UnpaidScreenState extends ConsumerState<UnpaidScreen> {
  ...
  Future<void> _printList(List<Map<String, dynamic>> students, Map<String, dynamic> globalStats) async {
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.TableHelper.fromTextArray(
            headers: ["N°", "Matricule", "Noms", "Classe", "À Payer", "Perçu", "Reste"],
    ...
    await Printing.layoutPdf(onLayout: (_) => pdf.save(), name: 'Impayes_${DateTime.now().toIso8601String()}.pdf');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const Text("Suivi des Impayés & Rapports", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 8),
          const Text("Vue globale sur les recouvrements, filtrage par salle et impression.", style: TextStyle(color: AppColors.textSecondary, fontSize: 16)),
              ...
              return Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _StatCard(title: "Élèves (Vue actuelle)", value: "${students.length}", icon: Icons.groups, color: Colors.blue),
                      _StatCard(title: "Montant Attendu", value: "$viewExpected", icon: Icons.track_changes, color: Colors.purple),
                      _StatCard(title: "Montant Perçu", value: "$viewReceived", icon: Icons.account_balance_wallet, color: Colors.green),
                      _StatCard(title: "Reste à Recouvrer", value: "${viewExpected - viewReceived}", icon: Icons.money_off, color: Colors.red),
                    ],
                  ),
                  const SizedBox(height: 32),
                          ...
                          children: [
                            Expanded(
                              ...
                              child: CustomTextField(
                                ...
                                hint: "Tapez pour filtrer les élèves affichés ci-dessous...",
                            ...
                            ElevatedButton.icon(
                              ...
                              label: const Text("Imprimer Liste", style: TextStyle(color: Colors.white)),
                              ...
                              onPressed: () => _printList(students, globalStats),
                            )
                          ],
                        ),
                      ],
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: DataTable(
                        ...
                        columns: const [
                          ...
                          DataColumn(label: Text("Attendu", style: TextStyle(fontWeight: FontWeight.bold))),
                          DataColumn(label: Text("Perçu", style: TextStyle(fontWeight: FontWeight.bold))),
                          DataColumn(label: Text("Reste", style: TextStyle(fontWeight: FontWeight.bold))),
                        ...
                        rows: students.map((s) {
                          ...
                          return DataRow(
                            cells: [
                              DataCell(Text(s['matricule'], style: const TextStyle(fontWeight: FontWeight.w500))),
                              DataCell(Text("${s['prenom']} ${s['nom']}")),
                              DataCell(Text(s['classe_assignee'] ?? '')),
                              DataCell(Text("${s['total_expected']}")),
                              DataCell(Text("${s['total_paid']}", style: const TextStyle(fontWeight: FontWeight.bold))),
                              DataCell(Text("${s['remaining']}", style: TextStyle(color: isSolde ? Colors.green : Colors.red, fontWeight: FontWeight.bold))),
                              DataCell(
                                Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                  decoration: BoxDecoration(color: isSolde ? Colors.green : Colors.orange, borderRadius: BorderRadius.circular(12)),
                                  child: Text(isSolde ? "SOLDE" : "EN DETTE", style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                                )
                              ),
                            ]
                          );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
